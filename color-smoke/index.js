jQuery(function($) {
	...
	$.fn.colorSmoke = function(Options, Callback) {
	  ...
	  Blip.prototype.render = function(i) {
		...
		if (Settings.fixedColor) {
			ctx.fillStyle = 'hsla(' + rand(Settings.colorHue - Settings.maxHueDifference, Settings.colorHue + Settings.maxHueDifference) + ', 50%, 1%, ' + this.alpha + ')';
		} else {
			ctx.fillStyle = 'hsla(' + rand(globalTick - Settings.maxHueDifference, globalTick + Settings.maxHueDifference) + ', 50%, 1%, ' + this.alpha + ')';
		}
		return ctx.fill();
	  };
	...
	//$("canvas").eq(0).colorSmoke( {	fixedColor: true, colorHue: 255, maxHueDifference: 80 });
	//$("canvas").eq(1).colorSmoke();
});